...
#else
	#define USB_VENDOR_ID 0x11ff	//0x0483
	#define USB_PRODUCT_ID 0x3331 //0x5710
#endif

#define BULK_ENDPOINT_OUT 1
#define BULK_ENDPOINT_IN  0
...
#if 1
		cv::Point m_to;
		static unsigned long long int t1=0;
		while(1)
		{
			//printf(" cost time : %u \n",(cv::getTickCount() - t1)/1000000);
			t1 =  cv::getTickCount();
			rv =  libusb_interrupt_transfer(g_usb_handle,user_device.bInEndpointAddress,a,8,&length,0);
